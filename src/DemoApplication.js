...
DemoApplication.prototype.keyboardCallback = function(e){
  var key = e.keyCode;

  this.m_lastKey = 0;
  switch(key){
  case 'q': break; // Needed?
  case 'l' : this.stepLeft(); break;
  case 'r' : this.stepRight(); break;
  case 'f' : this.stepFront(); break;
  case 'b' : this.stepBack(); break;
  case 'z' : this.zoomIn(); break;
  case 'x' : this.zoomOut(); break;
  case 'i' : this.toggleIdle(); break;
  case 'g' : this.m_enableshadows=!this.m_enableshadows;break;
  case 'u' : this.m_shapeDrawer.enableTexture(!this.m_shapeDrawer.enableTexture(false));break;
    
    /*
    // Todo: Lookup bits and do this
  case 'h':
    if(this.m_debugMode & btIDebugDraw::DBG_NoHelpText)
